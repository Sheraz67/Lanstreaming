...
static constexpr const char* TAG = "AudioCaptureWASAPI";

// Noise gate: same threshold as PulseAudio backend
static constexpr float NOISE_GATE_THRESHOLD_SQ = 0.005f * 0.005f;

// Reference time units (100-ns intervals)
...
bool AudioCaptureWASAPI::init(uint32_t sample_rate, uint16_t channels) {
    ...
    enumerator->Release();
    if (FAILED(hr)) {
        LOG_ERROR(TAG, "Failed to get default render endpoint: 0x%08lx", hr);
        return false;
    }
    ...
    device_bits_ = static_cast<uint16_t>(mix_format->wBitsPerSample);

    LOG_INFO(TAG, "Device mix format: %u Hz, %u ch, %u bits",
             device_sample_rate_, device_channels_, device_bits_);

    // Initialize in shared mode with loopback flag
    REFERENCE_TIME buffer_duration = REFTIMES_PER_SEC / 10; // 100ms buffer
    ...
    // Set up resampler if device format differs from target
    if (device_sample_rate_ != sample_rate_ || device_channels_ != channels_) {
        ...
        LOG_INFO(TAG, "Resampler: %u Hz %u ch -> %u Hz %u ch",
    ...
    initialized_ = true;
    LOG_INFO(TAG, "WASAPI loopback capture initialized: %u Hz, %u channels",
             sample_rate_, channels_);
    return true;
}

std::optional<RawAudioFrame> AudioCaptureWASAPI::capture_frame() {
    if (!initialized_) return std::nullopt;

    size_t target_total = static_cast<size_t>(frame_samples_) * channels_;

    // Accumulate samples from WASAPI
...
void AudioCaptureWASAPI::shutdown() {
    ...
    initialized_ = false;
    LOG_INFO(TAG, "WASAPI loopback capture shut down");
}
